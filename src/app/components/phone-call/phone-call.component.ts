...
import { CallerType } from '../../models/index'
...
export class PhoneCallComponent implements OnInit {
	...
	async startPeerConnectionListeners(): Promise<void> {
		...
		try {
          ...
          if (event.candidate) this.sendICE(event.candidate, this.caller.remoteGuid)
				}
				catch (e) {
	...
	async startP2pConnection(): Promise<void> {
      ...
      if (this.pc.localDescription) await this.sendSDP(this.pc.localDescription, this.caller.remoteGuid);
			return;
		}
		catch (e) {
			throw (e);
	...
	public setHolded(isHolded: boolean) {
