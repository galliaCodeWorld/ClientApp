...
@Component({
  ...
  styleUrls: ['./other-rep.component.scss']
})
export class OtherRepComponent {
	...
	async sendPage(event: Event): Promise<void> {
		...
		try {
			...
			try {
				remoteGuid = await this.service.phoneSendPbxPhoneLineInvitation(
          this.otherRep.companyEmployee?.member ? this.otherRep.companyEmployee.member.email : "",
          this.otherRep.pbxLineRepId);
			}
			catch (e) {
				throw (e)
			}
		...
		try {
			...
			outgoingCallDialog
				...
				.subscribe(async (remoteGuid: string) => {
					...
					if (this.service.isEmpty(remoteGuid)) {
						try {
							await this.service.cancelCall(this.otherRep.companyEmployee?.member ? this.otherRep.companyEmployee.member.email : "");
						}
						catch (e) {
