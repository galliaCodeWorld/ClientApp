...
export class PbxLineComponent implements OnInit {
    ...
    let pbxLineId = id ? parseInt(id) : 0;
    ...
    this.service.initPhoneService(this.pbxLineRep.companyEmployee ? this.pbxLineRep.companyEmployee.title : "")
			.then(() => {
	...
	startListeners(): void {
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
			.subscribe((message: ObservableMessageType) => {
				...
				if (this.service.isEmpty(json) === false) {
                  ...
                  let comp = this.getPhoneCallComponentInstance(phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.connectionGuid : "");
                  if (comp) phoneCallComponent = comp;
								}
								return;
							})
							.then(() => {
                ...
                return this.service.sendReadyForCall(phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.connectionGuid : "");
							})
							.then(() => {
                ...
                this.service.sendNotReadyForCall(this.service.stringify(error), phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.connectionGuid : "");
							})
					}
				}
			});
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
            ...
            if (phoneLineConnection && phoneLineConnection.hubConnection) {
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
            ...
            smsMessage.sender = phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.name : "";
					}
					else {
						// TODO: received IceMessageType without sender string (remoteGuid), handle this error type
						console.log("received smsMessage without remoteGuid string (remoteGuid)");
	...
	removePhoneCallComponent(remoteGuid: any): void {
	...
	getPhoneCallComponentInstance(remoteGuid: string): PbxLineCallComponent | null {
		let index: number = this.phoneCallComponentRefs.findIndex((value) => {
			// NOTE: must use == equality instead of === for this to work, === will always return -1 because === doesn't work
			return value.instance.caller.remoteGuid == remoteGuid;
		})
	...
	addPhoneCallComponentToDom(phoneLineConnection: PhoneLineConnectionType): Promise<ComponentRef<PbxLineCallComponent>> {
		return new Promise<ComponentRef<PbxLineCallComponent>>((resolve) => {
			let index: number = this.phoneCallComponentRefs.findIndex((value) => {
				// NOTE: must use == equality instead of === for this to work, === will always return -1 because === doesn't work
        return value.instance.caller.remoteGuid == phoneLineConnection?.hubConnection?.connectionGuid ? phoneLineConnection.hubConnection.connectionGuid : "";
			})
			if (index < 0) {
        ...
        phoneCallComponent.caller.remoteGuid = phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.connectionGuid : "";
				phoneCallComponent.caller.profile = new ProfileDto();
        phoneCallComponent.caller.profile.email = phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.email : "";
        phoneCallComponent.caller.profile.name = phoneLineConnection.hubConnection ? phoneLineConnection.hubConnection.name : "";
	...
	displayOutgoingCall(email: string, duration = 30000) {
		let callingTimer: any;
