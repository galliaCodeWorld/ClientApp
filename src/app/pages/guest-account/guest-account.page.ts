	...
	async upgradeAccount(event: MouseEvent): Promise<void> {
        ...
        if (email) {
          ...
          model.email = email.value;
				...
				if (password) model.password = password.value;
				if (firstName) model.firstName = firstName.value;
				if (lastName) model.lastName = lastName.value;
	...
	async submit(event: MouseEvent): Promise<void> {
		...
		if (this.formGroup.valid) {
			...
			try {
				...
				try {
          localGuid = await this.service.webrtcHubCheckIn(this.profile.name);
				}
				catch (e) {
