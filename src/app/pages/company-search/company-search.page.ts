...
import { MatListOption, MatList, MatSelectionList } from '@angular/material/list';
...
export class CompanySearchPage {
	...
	profiles!: Array<ListItemType> | null;
	...
	set companyProfiles(value: Array<CompanyProfileDto>) {
        ...
        if (v.companyProfileId) item.id = v.companyProfileId.toString();
          ...
          : v.companyProfileId
            ...
            : this.service.defaultAvatar;
				if (v.companyName) item.title = v.companyName;
				if (v.description) item.content = v.description;
