...
export class NetcasteePage implements OnInit, AfterViewInit {
	...
	sendDataChannelMessage(): void {
		if (this.getRemoteGuidForm.valid) {
			try {
				...
				let netcast!: NetcastType;
        let find = this.netcastConnections.find((n: NetcastType) => {
          return n.remoteGuid == remoteGuid;
        });
        if (find) netcast = find;
	...
	async getPCOnly(): Promise<void> {
		...
		if (this.getRemoteGuidForm.valid) {
			try {
				this.showProgress = true;
				let remoteGuid = this.getRemoteGuidForm.get('remoteGuid')?.value;
				let incoming: NetcastType | undefined = this.initNetcast(remoteGuid, NetcastKind.incoming);
        if (incoming) {
          await this.service.getAccessToken();
	...
	async getNetcast(remoteGuid?: string): Promise<void> {
		if (this.service.isEmpty(remoteGuid)) {
          ...
          if (incoming) {
			...
			try {
				this.showProgress = true;
				let incoming: NetcastType | undefined;
        if (remoteGuid) incoming = this.initNetcast(remoteGuid, NetcastKind.incoming);
        if (incoming) {
          console.log("get Netcast from remoteGuid: ", incoming.remoteGuid);
          // call this.service.getAccessToken() to renew the accessToken
          // if needed. This also stores the accessToken in session or permanent storage
		...
		if (this.getRemoteGuidForm.valid) {
			try {
				...
				try {
					if (this.netcastConnections.length > 0) {
						...
						let incoming!: NetcastType;
            let find = this.netcastConnections.find((p: NetcastType) => {
            ...
            if (find) incoming = find;
						...
						if (this.service.isEmpty(incoming) === false) {
							...
							this.netcastConnections.forEach((n: NetcastType) => {
                if (incoming.mediaStream) this.switchMediaStream(n, incoming.mediaStream);
							})

							// display the video
              if (incoming.mediaStream)
	...
	async relayNetcast(remoteGuid: string): Promise<void> {
        ...
        if (find) relay = find;
          ...
          let outgoing: NetcastType | undefined = this.getPc(remoteGuid, this.netcastConnections);
					if (outgoing && this.service.isEmpty(outgoing) === false) {
            ...
            if (relay.mediaStream) this.switchMediaStream(outgoing, relay.mediaStream);
					}
					else {
            ...
            if (outgoing) {
              ...
              outgoing.mediaStream?.getTracks().forEach((t: MediaStreamTrack) => {
                if (outgoing?.mediaStream) outgoing.peerConnection?.addTrack(t, outgoing.mediaStream);
              })
              //pcType.rtpSender = rtpSender;
              //pc.addStream(currentPcType.mediaStream);
              let localSdpOffer: RTCSessionDescriptionInit | undefined = await outgoing.peerConnection?.createOffer();
			...
			.subscribe(async (message: ObservableMessageType) => {
				...
				try {
					...
					if (this.service.isEmpty(remoteGuid) === false) {
						...
						let nt: NetcastType | undefined = this.getPc(this.netcasterGuid, this.netcastConnections);
						if (nt && this.service.isEmpty(nt) === false) {
			...
			.subscribe(async (message: ObservableMessageType) => {
				...
				try {
					...
					if (this.service.isEmpty(json) === false && "remoteGuid" in json) {
						...
						let outgoing: NetcastType | undefined = this.initNetcast(remoteGuid, NetcastKind.outgoing);
            if (outgoing) {
              let localSdpOffer: RTCSessionDescriptionInit | undefined = await outgoing.peerConnection?.createOffer();
              await outgoing.peerConnection?.setLocalDescription(localSdpOffer);
			...
			.subscribe(async (message: ObservableMessageType) => {
				...
				try {
					...
					if (this.service.isEmpty(json) === false && "remoteGuid" in json) {
						...
						if (this.service.isEmpty(outgoing) === false && outgoing) {
							// get the first avaible video stream that is not the target connection
							let relay!: NetcastType;
              let find = this.netcastConnections.find((p) => {
	...
	initNetcast(remoteGuid: string, netcastKind: NetcastKind): NetcastType | undefined {
		try {
			// using existing netcast else create new
			let netcast: NetcastType | undefined = this.getPc(remoteGuid, this.netcastConnections);
					...
					if (event.streams && event.streams.length) {
						try {
							...
							if (this.service.isEmpty(netcast.videoElement) === false) {
								...
								this.netcastConnections.forEach((n: NetcastType) => {
									if (n.netcastKind === NetcastKind.outgoing && netcast.mediaStream) {
	...
	requestNetcastStub(r: RequestNetcastStubType, currentTry?: number): boolean {
		...
		if (currentTry <= maxTrys) {
			...
			let relayBranch: NetcastType | null = typeof outgoingRelays[relayIndex] !== "undefined" ? outgoingRelays[relayIndex] : null;
	...
	handleDisconnectEvent(remoteGuid: string): void {
		let netcast: NetcastType | undefined = this.getPc(remoteGuid, this.netcastConnections);
		if (netcast &&  this.service.isEmpty(netcast) === false) {
	...
	async handleSdpMessage(sdpMessage: SdpMessageType): Promise<void> {
		try {
			if (this.service.isEmpty(sdpMessage) === false && this.service.isEmpty(sdpMessage.sender) === false) {
				...
				if (sdp.type === "offer") {
					let incoming: NetcastType | undefined = this.getPc(remoteGuid, this.netcastConnections)!;
					...
					if (this.service.isEmpty(incoming) === false) {
						...
						let createAnswer = await incoming.peerConnection?.createAnswer();
						if (createAnswer) {
							localSdpAnswer = createAnswer;
	...
	async handleIceMessage(iceMessage: IceMessageType): Promise<void> {
        ...
        let n: NetcastType | undefined = this.getPc(iceMessage.sender, this.netcastConnections);
				if (n) {
	...
	removePc(netcast: NetcastType): NetcastType | null {
	...
	getPc(remoteGuid: string, netcasts: NetcastType[]): NetcastType | undefined {
	...
	async switchMediaStream(netcast: NetcastType, newMediaStream: MediaStream): Promise<void> {
        ...
        let find = tracks.find((t: MediaStreamTrack) => {
          return t.kind == s.track?.kind;
        });
				promises.push(
					s.replaceTrack(find ? find : null)
				);
			})
		}
