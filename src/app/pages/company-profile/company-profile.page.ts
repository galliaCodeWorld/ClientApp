...
export class CompanyProfilePage {
	...
	set companyProfile(value: CompanyProfileDto) {
      ...
      if (value.companyName) this.companyName = value.companyName;
      if (value.description) this.companyDescription = value.description;
      ...
      if (value.companyProfileId) this.companyProfileId = value.companyProfileId;
		}
	}
	...
	async init(companyProfileId: number): Promise<void> {
		try {
			...
			let idDto = new IdDto();
      if (companyProfile.companyProfileId)
