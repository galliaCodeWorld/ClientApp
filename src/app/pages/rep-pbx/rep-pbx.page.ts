...
import { filter, distinctUntilKeyChanged, distinctUntilChanged } from 'rxjs/operators';
...
export class RepPbxPage {
	...
	set companyProfile(value: CompanyProfileDto) {
      ...
      if (value.companyProfileId) this.genericCompanyProfile.id = value.companyProfileId.toString();
      if (value.companyName) this.genericCompanyProfile.name = value.companyName;
	...
	genericCompanyProfile!: GenericUserType | null;
	...
	set employee(value: CompanyEmployeeDto) {
      ...
      if (value.member) this.employeeProfile.name = value.member.firstName;
	...
	employeeProfile!: GenericUserType | null;
	...
	async initPage(pbxlineRepId: number): Promise<WarningsType> {
      ...
      if (this.pbxLineRep.pbxLine) {
        ...
        if (this.pbxLineRep.pbxLine.companyProfile) this.companyProfile = this.pbxLineRep.pbxLine.companyProfile;
        if (this.pbxLineRep.companyEmployee) this.employee = this.pbxLineRep.companyEmployee;
      }
				...
				try {
					...
					if (this.service.isEmpty(json) === false) {
						...
						if (this.service.isEmpty(phoneLineConnection) === false) {
							...
							let componentRef: ComponentRef<PhoneCallComponent> | null = await this.addPhoneCallComponentToDom(phoneLineConnection);
							if (this.service.isEmpty(componentRef) === false) {
                if (componentRef?.instance) phoneCallComponent = componentRef?.instance;
							}
              else if (phoneLineConnection.hubConnection) {
                let tmp = this.getPhoneCallComponentInstance(phoneLineConnection.hubConnection.connectionGuid);
                if (tmp) phoneCallComponent = tmp;
							}
							...
							if (this.service.isEmpty(phoneCallComponent) === false) {
								...
								try {
                  if (phoneLineConnection.hubConnection)
					...
					if (this.service.isEmpty(this.customerQueue) === false) {
						...
						if (this.service.isEmpty(queue) === false) {
							this.ngZone.run(async () => {
								...
								if (this.service.isEmpty(accessToken) === false && queue) {
							...
							if (this.service.isEmpty(this.customerQueue) === false) {
								...
								for (let i = 0; i < check.length; i++) {
									...
									if (index < 0) {
										...
										if (this.service.isEmpty(item) === false && item) {
	...
	*/
	...
	async addCustomer(queue: PbxCallQueueDto, accessToken: string): Promise<PbxCustomerType | null> {
			...
			dialogRef.componentInstance.onIncomingCallResponse.subscribe(async (response: boolean) => {
				...
				if (response === true) {
					...
					if (this.service.isEmpty(pbxlineRepStatus) === false) {
						try {
              if (pbxlineRepStatus.pbxLineRep)
