...
export class NetcasterPage implements OnInit, AfterViewInit {
	...
	startSubscriptions(): void {
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
			.subscribe(async (message: ObservableMessageType) => {
              ...
              if (mediaStream && this.service.isEmpty(mediaStream) === false) {
	...
	async handleSdpMessage(sdpMessage: SdpMessageType): Promise<void> {
		try {
			if (this.service.isEmpty(sdpMessage) === false && this.service.isEmpty(sdpMessage.sender) === false) {
				...
				if (sdp.type === "answer") {
					let n: NetcastType | undefined = this.getPc(sdpMessage.sender, this.netcastConnections);
					if (n) {
	...
	async requestNetcastStub(r: RequestNetcastStubType, currentTry?: number): Promise<boolean> {
		try {
			...
			if (currentTry <= maxTrys) {
				...
				let netcastBranch: NetcastType | null = typeof outgoingRelays[relayIndex] !== "undefined" ? outgoingRelays[relayIndex] : null;
				...
				if (this.service.isEmpty(netcastBranch)) {
					...
					let dc: RTCDataChannel | undefined = netcastBranch?.getDataChannel(DataChannelKind.dcJsonType);
					if (dc && this.service.isEmpty(dc) === false) {
          ...
          else if (netcastBranch?.remoteGuid) {
	...
	async switchCamera(): Promise<void> {
		try {
			...
			let mediaStream: MediaStream | undefined = this.mediaStreams.find((m: MediaStream) => {
				return m.id == this.netcastStreamId;
			});
      if (mediaStream) {
        ...
        this.netcastConnections.forEach((n: NetcastType) => {
          if (mediaStream) promises.push(this.switchMediaStream(n, mediaStream));
        });
