...
export class MeetingPage implements OnInit, AfterViewInit {
	...
	async enterMeeting(): Promise<void> {
      ...
      if (this.meeting.phoneLineId) phoneLine = await this.service.getPhoneLineById(this.meeting.phoneLineId);
			console.log("got phoneLine: ", phoneLine);
	...
	*/
	...
	startSubscriptions(): void {
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
			.subscribe(async (message: ObservableMessageType) => {
              ...
              let callCmpToDom = await this.addPhoneCallComponentToDom(phoneLineConnection);
              if (callCmpToDom) componentRef = callCmpToDom;
							if (this.service.isEmpty(componentRef) === false) {
								phoneCallComponent = componentRef.instance;
							}
              else if (phoneLineConnection.hubConnection?.connectionGuid) {
                let cmpInstance = this.getPhoneCallComponentInstance(phoneLineConnection.hubConnection.connectionGuid);
                if (cmpInstance) phoneCallComponent = cmpInstance;
							}
							...
							if (this.service.isEmpty(phoneCallComponent) === false) {
								...
								try {
                  if (phoneLineConnection.hubConnection?.connectionGuid)
					...
					// NOTE: fatal error
          if (this.service.isEmpty(phoneLineConnection) === false && phoneLineConnection.hubConnection?.connectionGuid) {
      ...
      .pipe(distinctUntilKeyChanged<ObservableMessageType>("timestamp"))
          ...
          let callCmpToDom = await this.addPhoneCallComponentToDom(phoneLineConnection);
          if (callCmpToDom) componentRef = callCmpToDom;
          if (this.service.isEmpty(componentRef) && phoneLineConnection.hubConnection?.connectionGuid) {
            let cmpInstance = this.getPhoneCallComponentInstance(phoneLineConnection.hubConnection.connectionGuid);
            if (cmpInstance) phoneCallComponent = cmpInstance;
					}
					else {
	...
	getPhoneCallComponentInstance(remoteGuid: string): PhoneCallComponent | null {
		let index: number = this.phoneCallComponentRefs.findIndex((value) => {
			// NOTE: must use == equality instead of === for this to work, === will always return -1 because === doesn't work
			return value.instance.caller.remoteGuid == remoteGuid;
		})
	...
	async addPhoneCallComponentToDom(phoneLineConnection: PhoneLineConnectionType): Promise<ComponentRef<PhoneCallComponent> | null> {
		try {
			let componentRef: ComponentRef<PhoneCallComponent>;
			let index: number = this.phoneCallComponentRefs.findIndex((value) => {
				// NOTE: must use == equality instead of === for this to work, === will always return -1 because === doesn't work
        return phoneLineConnection.hubConnection?.connectionGuid && value.instance.caller.remoteGuid == phoneLineConnection.hubConnection.connectionGuid;
			})
			if (index < 0) {
				let member!: MemberType;
				try {
          if (phoneLineConnection.hubConnection?.email) {
        ...
        if (phoneLineConnection.hubConnection?.connectionGuid) phoneCallComponent.caller.remoteGuid = phoneLineConnection.hubConnection.connectionGuid;
        ...
        else phoneCallComponent.caller.profile.email = member.email;
        if (this.service.isEmpty(member)) {
          if (phoneLineConnection.hubConnection?.name)
            phoneCallComponent.caller.profile.name = phoneLineConnection.hubConnection.name;
        }
        else phoneCallComponent.caller.profile.name = member.firstName + " " + member.lastName;
				...
				let genericUser = new GenericUserType();
        if (phoneLineConnection.hubConnection?.connectionGuid) genericUser.id = phoneLineConnection.hubConnection.connectionGuid;
        if (this.service.isEmpty(member)) {
          if (phoneLineConnection.hubConnection?.email)
            genericUser.email = phoneLineConnection.hubConnection.email;
        }
        else genericUser.email = member.email;
        if (this.service.isEmpty(member)) {
          if (phoneLineConnection.hubConnection?.name)
            genericUser.name = phoneLineConnection.hubConnection.name;
        }
        else genericUser.name = member.firstName + " " + member.lastName;
          ...
          return phoneLineConnection.hubConnection?.connectionGuid && user.id === phoneLineConnection.hubConnection.connectionGuid;
				});
				if (index < 0) {
					this.currentUsers.push(genericUser);
					this.currentUsers = this.currentUsers.slice();
