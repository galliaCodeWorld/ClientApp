...
export class Service {
	...
	urlB64ToUint8Array(base64String: any): Uint8Array {
		...
		if (this.isEmpty(this.phoneLine)) {
			...
			if (this.isEmpty(this.phoneLine.phoneLineConnections)) {
				...
				let index = this.phoneLine.phoneLineConnections.findIndex((value) => {
					return value.hubConnection && value.hubConnection.connectionGuid != this.localGuid;
				});
	...
	getPhoneLineConnectionFromCacheByRemoteGuid(remoteGuid: string): PhoneLineConnectionType {
		...
		let phoneLineConnection: PhoneLineConnectionType = this.phoneLine.phoneLineConnections.find((value) => {
			return value.hubConnection && value.hubConnection.connectionGuid == remoteGuid && value.hubConnection.isDeleted == false;
		})!;
			...
			if (phoneLineConnection.phoneLineId === this.phoneLine.phoneLineId && phoneLineConnection.isDeleted === false) {
				if (this.isEmpty(this.phoneLine.phoneLineConnections)) {
					...
					let index: number = this.phoneLine.phoneLineConnections.findIndex((value) => {
						...
						return value.hubConnection && phoneLineConnection.hubConnection && value.hubConnection.email == phoneLineConnection.hubConnection.email;
					})
	...
	async phoneSendPbxPhoneLineInvitation(email: string, pbxLineRepId: number): Promise<string> {
		...
		try {
			...
			let remoteGuid: string = await this.signalrService.sendPbxPhoneLineInvitation(this.phoneLine.phoneLineGuid, pbxLineRepId, email,
        this.localPhoneLineConnection.hubConnection ? this.localPhoneLineConnection.hubConnection.name : '');
			return remoteGuid;
		}
		catch (e) {
			throw (e);
	...
	async phoneSendPhoneLineInvitation(email: string): Promise<string> {
		...
		try {
			...
			try {
				remoteGuid = await this.sendPhoneLineInvitation(this.phoneLine.phoneLineGuid, email,
          this.localPhoneLineConnection.hubConnection ? this.localPhoneLineConnection.hubConnection.name : '');
			}
			catch (e) {
				throw (e);
	...
	*/
				...
				if (this.isPhoneBusy() === false) {
					...
					if (isMember) {
						...
						let index = call.callers?.findIndex((value) => {
							return this.isBlockedEmailFromCache(value.profile ? value.profile.email : '')
						});
						isBlocked = index !== undefined ? (index > -1) : false;
					}
					if (isBlocked) {
	...
	async callContact(email: any): Promise<void> {
	...
	async displayOutgoingCall(email:any, duration = 60): Promise<void> {
	...
	*/
	...
	sendKeepAlive(email: string, connectionId: any): Promise<void> {
	...
	*/
	...
	async checkMemberCookieLogin(): Promise<void> {
		...
		if (rememberMe == 'true' && !this.isEmpty(cookieJwtToken)) {
			...
			let parsedCookieJwtToken: any = JSON.parse(cookieJwtToken);
	...
	async isCheckedIntoHubConnection(): Promise<boolean> {
		...
		try {
			...
			let profile: GenericUserType | null = null;
			let isMember = await this.isMember();
			...
			if (!this.isEmpty(profile)) {
				...
				if (this.isEmpty(isEmailReady)) {
					...
					await this.setEmail(profile ? profile.email : '');
				}
				else {
